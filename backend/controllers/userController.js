...
// payment gateway for appointment using Stripe 
...
const paymentStripe = async (req,res) => {
  ...
  try {
    ...
    const line_items = [
      {
        price_data: {
          currency,
          product_data: {
            name: `Appointment with Dr. ${appointment.docData?.name || appointment.docId}`,
          },
    ...
    console.error("paymentStripe error:", error);
...
const verifyStripe = async (req,res) =>{
    ...
    if (success === "true") {
      ...
      // Payment failed or cancelled — you may choose to revert or leave as is
      return res.json({ success: false, message: "Payment was cancelled or failed" });
    }
  } catch (error) {
    console.error("verifyStripeAppointment error:", error);
    res.status(500).json({ success: false, message: error.message });
  }
} 
